...
| `v`       | **Full cell** popup — `y` copy; `f` pretty-print valid JSON (press again for raw); while JSON mode is on, each cell you move to with `h`/`l`/`j`/`k` is pretty-printed when valid; `h`/`l` or `←`/`→` adjacent column; `j`/`k` or `↑`/`↓` adjacent row; long lines wrap to the popup width; `PgDn`/`PgUp`, `ctrl+f`/`ctrl+b`, and `g`/`G` scroll when content exceeds the viewport; `esc` close                |
