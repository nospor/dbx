...
- **`max_results_context_kb`** — maximum size (in KB) of the **query + result rows** block appended to the outbound prompt when you use **`/results`** in the AI pane. Default is **0** (unlimited / no limit). If set to a positive value, long SQL is truncated and rows are cut off with a truncation note if the limit is reached.
...
To prevent operating system "argument list too long" errors (which can easily occur when sending large database schemas or results), dbx writes the full outbound prompt to a temporary file (e.g. `dbx-ai-prompt-*.txt`) in the AI agent's working directory. It then passes an instruction to the AI CLI pointing to this file path (e.g., `"In file /path/to/dbx-ai-prompt-*.txt there are instructions to do with needed informations."`). These files are kept in the working directory for history and debugging purposes.
